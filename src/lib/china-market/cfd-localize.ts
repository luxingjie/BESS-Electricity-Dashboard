/**
 * Localize BNEF CfD workbook English fragments for China-facing UI.
 * Prefer applying at import time; display helpers remain as a safety net.
 */
...
const POT_PHRASES: Array<[RegExp, string]> = [
...
function localizeUsDate(raw: string): string {
...
export function localizeCommissioningWindow(
  value: string | null | undefined,
): string | null {
  if (!value) return null;
  const parts = value.split("-").map((part) => part.trim());
  if (parts.length === 2 && parts.every((part) => /^\d{1,2}\/\d{1,2}\/\d{4}$/.test(part))) {
    return `${localizeUsDate(parts[0])}–${localizeUsDate(parts[1])}`;
  }
  return value;
}

export function localizePotDesign(
  value: string | null | undefined,
): string | null {
  if (!value) return null;
  let text = value;
...
export function localizeCfdNote(value: string | null | undefined): string | null {
  if (!value) return null;
  return value
    .split("；")
    .map((segment) => {
      ...
      // Tolerate minor punctuation drift from Excel.
      const matched = Object.entries(NOTE_ZH).find(
        ([english]) =>
          english.replace(/[.’']/g, "'") === trimmed.replace(/[.’']/g, "'"),
      );
...
export function localizeSourceName(
  value: string | null | undefined,
): string | null {
  if (!value) return null;
